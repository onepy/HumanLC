...
class HumanEmulator(Plugin):
    ...
    def _load_config(self):
        ...
        if os.path.exists(config_path):
            logger.debug(f"[HumanEmulator] loading config from {config_path}")
            with open(config_path, "r", encoding="utf-8") as f:
                return json.load(f)
        elif os.path.exists(template_path):
            logger.debug(f"[HumanEmulator] loading config from template {template_path}")
            with open(template_path, "r", encoding="utf-8") as f:
               return json.load(f)
        else:
            logger.debug("[HumanEmulator] no config file, use default config")
            return {} # 如果不存在配置文件，返回一个空的字典
        ...
        try:
            with self._lock:
                ...
                for session_id, session_data in self.private_sessions.items():
                    ...
                    if (
                        time_diff >= self.PRIVATE_MSG_TIMEOUT
                        or len(session_data["messages"]) >= self.PRIVATE_MSG_THRESHOLD
                    ):
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if e_context["context"].get("isgroup", False):
            ...
            self.message_cache[session_id].append((sender_id, message, timestamp))
    ...
    def on_generate_reply(self, e_context: EventContext):
       ...
       if e_context["context"].get("isgroup", False):
           ...
           e_context.action = EventAction.BREAK_PASS # 跳过默认逻辑
       else:
           # 处理私聊消息，交给定时器处理
           e_context.action = EventAction.CONTINUE
       return
...
"
        ...
        if e_context:
            ...
            memory.user_session(session_id).set(all_messages)
        else:
             memory.user_session(session_id).set(all_messages)
    ...
    def _clean_cache(self, session_id):
        ...
        self.message_cache[session_id] = [
            item
            for item in self.message_cache[session_id]
            if current_time - item[2] <= self.CACHE_EXPIRY_TIME
        ]
